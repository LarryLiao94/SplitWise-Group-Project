...
const RemoveFriendForm = () => {
  ...
  const onClick = async (e) => {
    e.preventDefault();
    dispatch(removeFriendThunk(Number(id)));
  ...
  return (
    <>
      ...
      <div className="remove-friend-container">
        <h3 className="remove-friend-title">Edit friend info</h3>
        {/* {friend.email} */}
        <span className="remove-friend-text">
          Some of your expenses with this person also involve other third
          parties.
          <span className="remove-friend-text-bold">
            As a result, deleting this friend will not delete those expenses,
            and they will still be visible from the "All expenses" screen.
          </span>
          However, this friend should be removed from your list of friends
          successfully.
        </span>
        <div className="remove-friend-buttons">
          <button className="remove-friend-cancel" onClick={onCancel}>
            Cancel
          </button>
          <button className="remove-friend-delete" onClick={onClick}>
            Delete this friend
          </button>
        </div>
      </div>
